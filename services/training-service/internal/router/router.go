...
func NewRouter(
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Route("/trainings", func(r chi.Router) {
			...
			r.With(middleware.AuthMiddleware(jwtSecret), middleware.RequireRoles("UMKM")).Post("/enroll", trainingHandler.EnrollUser)
		})
		...
		r.Route("/enrollments", func(r chi.Router) {
			...
			r.With(middleware.RequireRoles("UMKM")).Patch("/progress", trainingHandler.UpdateProgress)
			r.With(middleware.RequireRoles("UMKM")).Patch("/complete", trainingHandler.CompleteTraining)
		})

		// Certificate endpoints (protected)
		r.Route("/certificates", func(r chi.Router) {
			r.Use(middleware.AuthMiddleware(jwtSecret))
			r.With(middleware.RequireRoles("ADMIN")).Get("/list", certHandler.ListCertificates)
			r.With(middleware.RequireRoles("ADMIN")).Get("/stats", certHandler.GetCertificateStats)
			r.With(middleware.RequireRoles("ADMIN")).Post("/{id}/approve", certHandler.ApproveCertificateByAdmin)
			r.With(middleware.RequireRoles("ADMIN")).Post("/{id}/reject", certHandler.RejectCertificate)
			...
			r.With(middleware.RequireRoles("UMKM")).Post("/request", certHandler.RequestCertificate)
		})
