...
<!-- STABILIZE_DEV_MILESTONE_START -->
## Integrated Local Development Snapshot

This branch uses the integrated local microservice layout below.

| Component | Local URL / Port | Notes |
|---|---:|---|
| Frontend | http://localhost:5173 | Vite dev server |
| auth-service | http://localhost:8080/api/v1 | Login, register, admin validation |
| user-service | http://localhost:8081/api/v1 | UMKM/Mitra profiles, products, sales reports |
| partnerships-service | http://localhost:8082/api/v1 | Partnership requests and review flow |
| document-service | http://localhost:8083/api/v1 | Upload metadata and object-storage integration |
| training-service | http://localhost:8084/api/v1 | Training, evaluation, certificates |
| Garage public endpoint | http://localhost:3900 | S3-compatible object storage |
| PostgreSQL | localhost:5432 | Main local database, unless changed in `.env` |
...
```text
8080 auth-service          200 OK
8081 user-service          200 OK
8082 partnerships-service  200 OK
8083 document-service      200 OK
8084 training-service      200 OK
```
...
cd ../partnerships-service && go test ./...
cd ../document-service && go test ./...
cd ../training-service && go test ./...
```

### Newman API regression check

```bash
newman run tests/postman/umkm-tumbuh-current-progress.postman_collection.json \
  -e tests/postman/local.postman_environment.json \
  -r cli,htmlextra \
  --reporter-htmlextra-export tests/postman/reports/stabilize-dev-milestone-report.html
```

Generated HTML reports under `tests/postman/reports/` are optional artifacts. Do not commit new generated reports unless the team intentionally wants to keep them.
<!-- STABILIZE_DEV_MILESTONE_END -->
